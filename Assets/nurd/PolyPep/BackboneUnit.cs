...
public class BackboneUnit : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        {
            if (transform.parent.GetComponent<Residue>() != null)
            {
                myResidue = transform.parent.GetComponent<Residue>();
            }
        }
        // myResidue = (gameObject.transform.parent.gameObject.GetComponent("Residue") as Residue);
        myPolyPepBuilder = transform.parent.parent.GetComponent<PolyPepBuilder>();
